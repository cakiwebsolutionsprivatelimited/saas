...
const getAppTheme = (appId: string) => {
  switch (appId) {
    ...
    case 'field-service': return { icon: Truck, classes: 'text-stone-700 bg-warm-sand group-hover:bg-stone-800 group-hover:text-white' };
    ...
    default: return { icon: LayoutGrid, classes: 'text-stone-700 bg-warm-sand group-hover:bg-stone-800 group-hover:text-white' };
  }
};
...
export default function FeaturedAppsSection() {
  return (
    <Section className="bg-warm-cream py-16 sm:py-20 border-t border-warm-sage">
      <Container>
        {/* Section Header */}
        <motion.div 
          ...
          {CATEGORIES.map((category) => {
            ...
            return (
              <motion.div 
                key={category.id} 
                variants={fadeUp}
                className="break-inside-avoid-column mb-8 rounded-[2rem] bg-white p-8 shadow-sm ring-1 ring-warm-sage transition-all hover:-translate-y-1 hover:shadow-md hover:ring-indigo-300"
              >
                {/* Category Header */}
                  ...
                  {categoryApps.map(app => {
                    ...
                    return (
                      <Link 
                        key={app.id}
                        to={`/apps/${app.id}`}
                        className="group flex items-start gap-4 rounded-lg p-4 transition-all hover:bg-warm-cream border border-transparent hover:border-warm-sage hover:shadow-sm"
                      >
                        ...
                        <div className="flex-grow pt-0.5">
                          <div className="flex flex-wrap items-center gap-2 mb-1">
                            ...
                            {app.badge && (
                              <span className="inline-flex items-center rounded-md border border-warm-sage bg-warm-sand px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-stone-700">
                                {app.badge}
                              </span>
                            )}
                          </div>
                          <p className="text-sm text-stone-600 line-clamp-2 leading-relaxed font-medium">
                            {app.description}
                          </p>
                        </div>
                      </Link>
                ...
                <div className="mt-6 pt-6 border-t border-warm-sage flex justify-end">
